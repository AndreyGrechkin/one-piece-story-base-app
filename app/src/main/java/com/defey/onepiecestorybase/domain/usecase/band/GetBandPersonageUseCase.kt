...
class GetBandPersonageUseCase(
    ...
    override fun execute(parameters: Int?): Flow<Response<List<PersonageBandContent>>> {
        ...
        return repo.getBandPersonageByBand(param).map { bandPersonageList ->
           val personageList = personageRepo.getPersonageInBand(bandPersonageList.map { it.personageId })
            val personageDescriptionList = personageDescriptionRepo.getPersonageDescriptionInPlace(personageList.map { it.id })
           val contentList = personageList.map {personage ->
               val personageDescription = personageDescriptionList.filter { it.personageId == personage.id}
               val image = personageDescriptionList.filter { it.personageId == personage.id }.findLast { it.image !=null }?.image
